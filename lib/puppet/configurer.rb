...
class Puppet::Configurer
  ...
  def last_agent_environment
    if Puppet::FileSystem.exist?(Puppet[:lastrunfile])
      ...
      last_environment = summary.dig('application', 'environment')
    end

    Puppet.debug(_("Found last used environment: %{environment}") % { environment: last_environment }) if last_environment
    last_environment
  rescue => detail
